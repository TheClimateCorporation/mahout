...
public class LogisticModelParameters implements Writable {
	...
	public OnlineLogisticRegression createRegression() {
		if (lr == null) {
			PriorSGDStrategy sgdStrategy = new PriorSGDStrategy(new L1());
			sgdStrategy.setLambda(getLambda());
			lr = new OnlineLogisticRegression(getMaxTargetCategories(), getNumFeatures(), sgdStrategy)
